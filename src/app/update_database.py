...
class UpdateDatabase:
	...
	def __init__(self, host, database_name, user, password, table_name):
		...
		self.dateFormats = ['%m/%d/%y', '%m/%d/%Y', '%Y-%b-%d', '%Y-%m-%d', '%d-%b-%y', '%y-%b-%d',
		...
		self._getTableHeaders() #store existing table headers in self.existingHeaders
	...
	def _hardcode_cases(self):
		if 'VL' in self.datatypes:
			self.datatypes['VL'] = 'int'
	...
	def _insert_row(self, data_list, header_list):
		...
		for index, _header in enumerate(header_list):
			...
			if data == " " or data == "":
				...
				if self.datatypes[header_list[index]] == "int" or self.datatypes[header_list[index]] == "int(11)" or self.datatypes[header_list[index]] == "float(12)" or self.datatypes[header_list[index]] == "float":
					if data.isdigit():
						pass
					...
					elif header_list[index] == 'VL': #handle special case of VL
						if '<' in data:
				...
				#handle case where a value in a text column should be translated to null
				elif data.lower() in self.codify:
			...
			if data == "NULL" or self.datatypes[header_list[index]] == "int" or self.datatypes[header_list[index]] == "float":
				add = " " + data + ", "
	...
	def _format_search(self, column_name):
		...
		if self.datatypes[column_name] == "int" or self.datatypes[column_name] == "int(11)" or self.datatypes[column_name] == "float":
			return self._format_num_search(column_name)
		elif self.datatypes[column_name] == "text":
	...
	def _format_varchar_search(self, column_name):
		...
		if len(column_data) <= 10 or column_name.lower() == 'study_name':
...
def main():
	...
	DatabaseObj.add_txt('Noguera-Julian_PRJNA307231-info.txt')
	# print(DatabaseObj._determine_file_type('Li_ERP113874-info.txt'))
	# print(DatabaseObj._determine_file_type('table.csv'))
	# print(DatabaseObj._determine_file_type('Villanueva-Millan-info.xlsx'))

	#DatabaseObj.search([('HBV_coinfection', " = 'positive'"), ('experiment', "= 'SRX1525257'")])
	#print(DatabaseObj.create_format_dict())
